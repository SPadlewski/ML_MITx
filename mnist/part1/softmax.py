...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    M=H[Y,range(n)]
    #taking the log of the prob matrix
    error_mean=np.mean(np.log(M))*(-1)
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    gradj = (-1 / (temp_parameter * n)) * np.dot((M - H), X) + (lambda_factor * theta)
    ...
    H = compute_probabilities(X, theta, temp_parameter)
    M = sparse.coo_matrix(([1]*n, (Y,range(n))), shape=(k,n)).toarray()
    non_regularized_gradient = np.dot(M-H, X)
    non_regularized_gradient *= -itemp/n
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    #YOUR CODE HERE
    Y=np.mod(Y,3)
    pred_Y=np.mod(get_classification(X,theta,temp_parameter),3)

    return 1-np.mean(Y==pred_Y)
