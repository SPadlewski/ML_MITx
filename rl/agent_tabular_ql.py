...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    """
    # Decide how to action based on the epsilon probability
    if np.random.binomial(1, epsilon):
        # Select action and object randomly from all possible action and objects (NUM_ACTIONS,NUM_OBJECTS)
        action_index,object_index=np.random.randint(NUM_ACTIONS, size=1), np.random.randint(NUM_OBJECTS, size=1)

    else:
        # Getting linear indexes of the action and object maximizing Q at state 1 and state2 and using unravel to change them back to 2D tuple
